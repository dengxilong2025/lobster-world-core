...
## 8) v0.2-M2：批量 world_id 管理策略（避免污染）

> 目标：让批测可重复、可对比，避免不同批次互相污染结果。

### 8.1 推荐命名规则

- **人工演示/手动测试**：继续使用固定的 `w1`（方便沟通与复现）
- **批量智能体测试**：使用“带时间戳/批次号”的 world_id，例如：
  - `agent_20260421_01`
  - `agent_20260421_02`

这样每轮批测可以用独立 world，避免历史事件影响 `home` 与 `export`。

### 8.2 清理策略（v0 现状）

当前 EventStore/Sim 状态为**内存态**：
- **重启服务**即可清空所有 world 的状态与事件
- 若使用 `docker compose up`：执行 `docker compose down` 后再 `up --build` 即相当于清空

（后续如果引入持久化 EventStore，会在文档中补充对应的清理/归档策略。）
